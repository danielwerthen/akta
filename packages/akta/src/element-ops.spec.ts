...
describe('Element ops', () => {
  it('should handle observable classNames', async () => {
    ...
    const unsub = render(jsx('div', { className: subj }), root);
    subj.next('cl454');
  ...
  it('should handle onclick events', async () => {
    ...
    const subj = new Subject();
    const unsub = render(jsx('button', { onClick: subj }), root);
  ...
  it('should handle onclick functions', async () => {
    ...
    document.body.append(root);
    const fun = jest.fn();
    const unsub = render(jsx('button', { onClick: fun }), root);
    const btn = document.getElementsByTagName('button')[0];
    btn.click();
    btn.click();
    btn.click();
  ...
  it('should handle aria attributes', async () => {
    const root = document.createElement('div');
    const unsub = render(jsx('button', { ['aria-hidden']: true }), root);
    expect(root).toMatchSnapshot();
    unsub();
  });
});
